#!/usr/bin/env python2
#Dependencies:
#sudo apt-get install python2.7 python-pygame python-cffi python-dev
#(python-dev looks like a bug, python-cffi should depend on that)
#plus a core
...
#envs used by snes9x:
#34 SET_SUBSYSTEM_INFO: Ignored. It just gives us some data we don't need.
#16 SET_VARIABLES: Ignored. We don't support variables.
...
#36 SET_MEMORY_MAPS: Ignored. We have no use for this.
#10 SET_PIXEL_FORMAT: Handled.
#17 GET_VARIABLE_UPDATE: Ignored. We don't support variables.
#15 GET_VARIABLE: Ignored. We don't support variables.
@ffi.callback("retro_environment_t")
def env(id, rawdata):
	if False: throw
	elif id == core.RETRO_ENVIRONMENT_SET_PIXEL_FORMAT:
		data = ffi.cast("const enum retro_pixel_format *", rawdata)
		core.pixelformat = data[0]
		print(core.pixelformat)
	#elif id >= 65536: print("Env E"+str(id&0xFFFF))
	#else:             print("Env "+str(id))
	return False
...
if not core.retro_load_game(game): exit(1)
game = None

size = ffi.new("struct retro_system_av_info*")
core.retro_get_system_av_info(size)


pygame.init()

window = pygame.display.set_mode((size.geometry.base_width, size.geometry.base_height))
c=255
while True:
